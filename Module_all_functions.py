...
def Split_dataframe_calibration(df_x:pd.DataFrame,df_y:pd.DataFrame,variable:str):
    ...
    """
    Funzione che divide i dataFrame in due parti, utili poi per la calibrazione.
    :param df_x:
    :param df_y:
    :param variable:
    :return:
...
def LinearCalibration(df_x:pd.DataFrame,df_y:pd.DataFrame,df_y_validation,variable:str) -> pd.DataFrame:
    """
    :param df_cal_x:
    :param df_cal_y:
    ...
    """
    variable_x = df_x[variable].values
    variable_y = df_y[variable].values

    b, a = np.polyfit(variable_y, variable_x, 1)
    variable_y_calibrated = a + b*df_y_validation[variable].values
    df_y_val_calibrated = pd.DataFrame({variable:variable_y_calibrated,'time':pd.to_datetime(df_y_validation['time'].values)})
    #print("Coefficient a = ",a)
    #print("Coefficient b = ",b)
    return df_y_val_calibrated,b,a
...
def DeltaCalibration(df_calibration_obs:pd.DataFrame, df_calibration_model:pd.DataFrame,df_validation_model:pd.DataFrame,variable):
    ...
    df_y_val_calibrated = df_validation_model[variable].values + DeltaFactor
    #print("Delta factor = ",DeltaFactor)
    #print("Delta Factor = ",DeltaFactor)
    return pd.DataFrame({variable:df_y_val_calibrated})
...
def QM_Calibration(df_insitu:pd.DataFrame, df_sat:pd.DataFrame,variable,n):
    ...
    # creating labels for each quantiles
...
def FDM_correction(df_insitu:pd.DataFrame, df_sat:pd.DataFrame,variable,n):
    ...
    df_corrected_FM = pd.DataFrame(
        {variable: values_corrected_FM, 'orig_index': df_val_sat.loc[:, 'orig_index']}).sort_values(by='orig_index')
